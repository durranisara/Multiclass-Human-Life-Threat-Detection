...
if torch.__version__ < "2.6":
    raise ValueError("PyTorch version must be 2.6 or higher due to security vulnerability (CVE-2025-32434). Please upgrade using 'pip install --upgrade torch'.")
...
# Step 2: Tokenization - CHANGED TO DeBERTa
...
# Step 3: Model Setup - CHANGED TO DeBERTa
...
# Training Arguments - UPDATED OUTPUT DIRECTORIES
training_args = TrainingArguments(
    output_dir='./deberta_results',  # Changed directory
    num_train_epochs=4,
    per_device_train_batch_size=8,  # Reduced batch size for DeBERTa (memory intensive)
    per_device_eval_batch_size=8,   # Reduced batch size for DeBERTa
    warmup_steps=500,
    weight_decay=0.01,
    logging_dir='./deberta_logs',  # Changed directory
    logging_steps=10,
    eval_strategy='epoch',
    save_strategy='epoch',
    load_best_model_at_end=True,
    metric_for_best_model='accuracy',
    fp16=True,  # Enable mixed precision training for DeBERTa
)
...
try:
    ...
    model.to("cpu")
    # Disable fp16 when running on CPU
...
with open('deberta_results/train_results.json', 'w') as f:
    json.dump({'metrics': train_metrics, 'history': train_history}, f, indent=4)
print("Training results saved to deberta_results/train_results.json")
...
print("DeBERTa Test Classification Report:")
print(class_report)
...
with pd.ExcelWriter('deberta_results/test_results.xlsx') as writer:  # Changed path
...
print("Test results saved to deberta_results/test_results.xlsx")
...
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.title('DeBERTa Training Loss Curve')  # Updated title
plt.legend()
plt.savefig('deberta_results/training_loss_curve.png')  # Updated path
plt.show()
...
plt.ylabel('True')
plt.title('DeBERTa Confusion Matrix')  # Updated title
plt.savefig('deberta_results/confusion_matrix.png')  # Updated path
plt.show()
...
plt.title('Class Distribution in Training Set (DeBERTa)')  # Updated title
plt.savefig('deberta_results/class_distribution.png')  # Updated path
plt.show()

print("Graphs saved as PNG files in deberta_results folder.")  # Updated message