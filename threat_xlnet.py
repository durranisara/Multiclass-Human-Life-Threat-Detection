...
with open('xlnet_results/train_results.json', 'w') as f:
    json.dump({'metrics': train_metrics, 'history': train_history}, f, indent=4)
print("Training results saved to xlnet_results/train_results.json")
...
with pd.ExcelWriter('xlnet_results/test_results.xlsx') as writer:
...
print("Test results saved to xlnet_results/test_results.xlsx")
...
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.title('XLNet Training Loss Curve')
plt.legend()
plt.savefig('xlnet_results/training_loss_curve.png')
plt.show()
...
plt.savefig('xlnet_results/confusion_matrix.png')
plt.show()
...
plt.title('Class Distribution in Training Set (XLNet)')
plt.savefig('xlnet_results/class_distribution.png')
plt.show()

print("Graphs saved as PNG files in xlnet_results folder.")