...
# Step 2: Tokenization - CHANGED TO ALBERT
...
# Step 3: Model Setup - CHANGED TO ALBERT
...
training_args = TrainingArguments(
    output_dir='./albert_results',  # Changed directory
    num_train_epochs=4,
    per_device_train_batch_size=16,
    per_device_eval_batch_size=16,
    warmup_steps=500,
    weight_decay=0.01,
    logging_dir='./albert_logs',  # Changed directory
    logging_steps=10,
    eval_strategy='epoch',
    save_strategy='epoch',
...
os.makedirs('albert_results', exist_ok=True)
with open('albert_results/train_results.json', 'w') as f:
    json.dump({'metrics': train_metrics, 'history': train_history}, f, indent=4)
print("Training results saved to albert_results/train_results.json")
...
print("ALBERT Test Classification Report:")
print(class_report)
...
with pd.ExcelWriter('albert_results/test_results.xlsx') as writer:  # Changed path
...
print("Test results saved to albert_results/test_results.xlsx")
...
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.title('ALBERT Training Loss Curve')  # Updated title
plt.legend()
plt.savefig('albert_results/training_loss_curve.png')  # Updated path
plt.show()
...
plt.ylabel('True')
plt.title('ALBERT Confusion Matrix')  # Updated title
plt.savefig('albert_results/confusion_matrix.png')  # Updated path
plt.show()
...
plt.title('Class Distribution in Training Set (ALBERT)')  # Updated title
plt.savefig('albert_results/class_distribution.png')  # Updated path
plt.show()

print("Graphs saved as PNG files in albert_results folder.")  # Updated message